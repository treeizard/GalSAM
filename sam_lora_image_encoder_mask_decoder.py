from segment_anything_draft import sam_model_registry
import math
import torch
import torch.nn as nn
from torch.nn.parameter import Parameter
from segment_anything_draft.modeling import Sam
...
class _LoRA_qkv(nn.Module):
    ...
    # Used for computing query, key and value tensors used in attention mechnism. 
    # Particularly in the contex of self-attention mechanisms in transformers.

    def __init__(
            # Initializes the module with several linear layers.
            self,
            qkv: nn.Module,
    ...
    def forward(self, x):
        # This line of code computes the queries, keys and values simultaneously
        # B: the batch size.
        # N: the sequence length.
        # org_c: the original number of features.
...
class _LoRA_qkv_proj(nn.Module):
    # The code below implements a mechnism for projecting input data and applying multiple linear transformations to it in a cacaded manner.
    # This approach allows the model to capture complex relationships and patterns within the data, potentially improving its representational power and performance on various tasks. 
...
class LoRA_Sam(nn.Module):
    ...
    Args:
        ...
        r: rank of LoRA
        -----------------Aside Begin---------------------
        rank: the order or importance assigned to different elements within the data. 
        
        1. Relevance Rank: the model learns to assign different ranks to different elements based on their relevance to
        the current context or task. Higher ranks indicate higher relevance or importance, whereas lower ranks indicate 
        lower relevance.

        2. Ranking Transformation: the learned transformations applied to the input data. 
        -----------------Aside End---------------------
    ...
    def __init__(self, sam_model: Sam, r: int, lora_layer=None):
        ...
        # create for storage, then we can init them or load weights (the weight that we will be modifying)
        self.w_As = []  # These are linear layers
        self.w_Bs = []

        # The below code freezes the encoder layer in the 
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            # t_layer_i: the index of the current layer (or block) in the loop.
            # blk: the actual block object at a given index. 
            # lora layer: the layers that we have defined above then:
            ...
            # Convert the current block's self attention into Lora Self Attention
        ...
        # Make most of the blocks in the mask_decoder to have requires_grad = False.
